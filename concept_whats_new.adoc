...
=== Verbesserte Workload-Sicherheit beim Laden des Bildschirms

Cloud Insights hat die Benutzerfreundlichkeit verbessert, da Bildschirme für Beobachtbarkeit und Workload-Sicherheit effizienter angezeigt werden. Dadurch verkürzt sich die Ladezeit, während Sie auf diese Funktionen navigieren. Beachten Sie, dass sich die URLs der Seite Workload Security geändert haben. Vorherige URLs werden automatisch zu den neuen URLs umgeleitet.
...
Der link:concept_kubernetes_performance_monitoring_and_map.html["_Kubernetes-Netzwerk-Performance und -Zuordnung_"] Die Funktion vereinfacht die Fehlerbehebung durch Zuordnen von Abhängigkeiten zwischen Kubernetes-Workloads. Es bietet Echtzeiteinblicke in Latenzen und Anomalien bei der Kubernetes-Netzwerk-Performance, um Performance-Probleme zu identifizieren, bevor sie sich auf die Benutzer auswirken. Diese Funktion hilft Unternehmen, durch Analyse und Prüfung des Kubernetes-Traffic-Flows die Gesamtkosten zu senken.
