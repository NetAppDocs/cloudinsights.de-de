...
Für das Management von Data Collectors (Add/Update/delete) und Agents stehen neue Workload Security APIs zur Verfügung. Navigieren Sie zu *Admin > API Access > API Documentation*, und wählen Sie den API-Typ _Workload Security_ aus, um weitere Informationen zu erhalten.
...
Die Workload-Sicherheit umfasst folgende Verbesserungen:

* Die Erkennungsalgorithmen wurden verbessert, um die Anzahl der falsch positiven Warnungen zu reduzieren.
* Die Leistung der Benutzeroberfläche wurde beim Laden, Anzeigen und Scrollen von Forensik-Seiten verbessert.
