...
=== Symbole Für Neue Objekte

Cloud Insights verfügt jetzt über eine aktualisierte Objekt-Ikonographie, wodurch die Gesamtvarianz der auf den verschiedenen Cloud Insights-Seiten verwendeten Symbole reduziert und ähnliche Objekte gruppiert werden (z. B. Volume-bezogene Objekte). Frühere „Letter-basierte“ Symbole werden durch bildbasierte Grafiken ersetzt, wodurch Cloud Insights-Bildschirme benutzerfreundlicher und zugänglicher werden.

Unsicher, was ein Symbol bedeutet? Die Tabelle unten (auch auf der Seite verfügbar link:reference_object_icons.html["Objekt Symbol Referenz"] ) erklärt sie alle.
...
Der link:task_config_telegraf_agent_k8s.html["Kubernetes Monitoring Operator"] (NKMO) ist die bevorzugte Methode für die Installation von Kubernetes für Cloud Insights Insights, für eine flexiblere Konfiguration der Überwachung in weniger Schritten und erweiterte Möglichkeiten zur Überwachung anderer Software, die im K8s-Cluster ausgeführt wird.
...
Es ist einfacher denn je, Ihre Datensammler mit neuen zu verfolgen link:task_system_monitors.html#data-collection-monitors["Meldungen"] Bei Fehlern der Datensammler- und Erfassungseinheit.
Beachten Sie, dass diese Monitore standardmäßig _Paused_ sind. Navigieren Sie zur Seite „Monitore“, und suchen Sie „Abschalten der Aufnahmeeinheit“ und „Collector failed“, und nehmen Sie sie wieder auf.
...
image:Variables_Drop_Down_Showing_Annotations.png["Dropdown-Anmerkungen in einer Variablen"]
image:Variables_Attribute_Filtering.png["Attributfilter in einer Variablen"]
...
|===
Details zur API-Verwendung finden Sie im link:API_Overview.html#api-documentation-swagger["API-Dokumentation"].
...
Die Installation von Kubernetes Agent wurde optimiert, damit weniger Benutzerinteraktionen erforderlich sind. link:task_config_telegraf_agent_k8s.html["Installieren des Kubernetes Agent"] Umfasst jetzt die Kubernetes-Datenerfassung.
...
* Cloud Insights unterstützt link:task_dc_amazon_ec2.html#collecting-from-aws-child-accounts["Erfassung aus AWS Child-Konten"] In einem einzelnen Datensammler gespeichert werden. Ihre AWS-Umgebung muss so konfiguriert sein, dass Cloud Insights den Abholung von untergeordneten Konten ermöglicht.
