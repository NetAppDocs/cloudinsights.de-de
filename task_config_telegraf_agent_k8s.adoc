...
Bearbeiten Sie die Bereitstellung des Monitoring-Operators in Operator-config.yaml, und ändern Sie alle Bildverweise, um Ihr privates Docker-Repository zu verwenden.
...
| Nach der Installation des Bedieners geben die telegraf-ds-Pods CrashLoopBackOff ein und die POD-Protokolle zeigen „su: Authentication failure“ an. | Bearbeiten Sie die konfiguration der netapp-Monitoring-Konfiguration und setzen Sie _dockerMetricCollectionEnabled_ auf false.  Weitere Informationen finden Sie im Abschnitt des Bedieners link:telegraf_agent_k8s_config_options.html["Konfigurationsoptionen"]. 
|===
