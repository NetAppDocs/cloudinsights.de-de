...
Wenn ein lokaler Root-SVM-Benutzer eine Aktivität ausführt, wird die IP des Clients, auf dem die NFS-Freigabe gemountet ist, im Benutzernamen berücksichtigt. Dieser wird auf den Seiten für forensische Aktivitäten und Benutzeraktivitäten als _root@<ip-address-of-the-client>_ angezeigt.
...
* Dies wird getan, um die Aktivität des NFS-Root-Benutzers nach IP-Adresse zu trennen. Zuvor wurde die gesamte Aktivität als vom _root_-Benutzer durchgeführt betrachtet, ohne IP-Unterscheidung.
