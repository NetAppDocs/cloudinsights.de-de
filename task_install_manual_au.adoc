...
. Hinweis: Wenn Sie planen, AU mit verschiedenen benutzerdefinierten Benutzern zu verwenden, stellen Sie sicher, dass Gruppenberechtigungen auf Lesen und Schreiben für Eigentümer und Gruppe eingestellt sind (_chmod 660 …_)
. Starten Sie den AU neu.
+
.. Navigieren Sie in Cloud Insights zu *Observability > Collectors* und wählen Sie die Registerkarte *Acquisition Units*. Wählen Sie _Neustart_ aus dem Menü „drei Punkte“ rechts neben der AU.
