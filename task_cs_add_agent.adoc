...
In jedem dieser Fälle wird der Agent bei der nächsten Aktualisierung der Workload-Sicherheit aktualisiert. Um automatische Agentenaktualisierungen anzuhalten oder fortzusetzen, verwenden Sie die APIs _cloudSecure_config.Agents_:

image:ws_pin_agent_apis.png["WS-PIN-Agent-APIs"]

Beachten Sie, dass es bis zu fünf Minuten dauern kann, bis die Aktion Pause oder Wiederaufnahme wirksam wird. Sie können Ihre aktuellen Agentenversionen auf der Seite Workload Security > Collectors auf der Registerkarte Agents anzeigen.

image:ws_agent_version.png["WS Agent-Versionen"]
