...
|  | SkipLockedMemory | Siehe das link:https://community.influxdata.com/t/updating-telegraf-to-version-1-29-5-crashes-kubernetes-pod/33376["Bekanntes Problem mit Telegraf"].  Wenn Sie skipLockedMemory einstellen, wird der Kubernetes Monitoring Operator angewiesen, Telegraf mit dem auszuführen `--unprotected` Flagge. 
