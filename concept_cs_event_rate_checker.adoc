...
| Frage: Wenn ich dieses Skript auf einer SVM lasse, die bereits für Workload Security konfiguriert ist, verwendet es nur die bestehende fpolicy-Konfiguration auf der SVM oder richtet es einen temporären ein und führt den Prozess aus? 


| Antwort: Der Event Rate Checker kann auch für eine SVM, die bereits für Workload-Sicherheit konfiguriert ist, problemlos ausgeführt werden. Es sollte keine Auswirkungen geben. 
