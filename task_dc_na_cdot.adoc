...
Cloud Insights kann ONTAP-Cluster-Switch-Daten als Option in den Collectors erfassen <<advanced-configuration,Erweiterte Konfiguration>> Einstellungen. Zusätzlich zur Aktivierung auf dem Cloud Insights-Collector müssen Sie auch *das ONTAP-System* selbst konfigurieren, um diese bereitzustellen link:https://docs.netapp.com/us-en/ontap-cli-98/system-switch-ethernet-create.html["Switch-Informationen"]Und sorgen Sie für die richtige <<a-note-about-permissions,Berechtigungen>> Werden eingestellt, um die Switch-Daten an Cloud Insights senden zu können.
...
| Kennzahlen Für Cluster-Switch | Zulassen, dass Cloud Insights Cluster-Switch-Daten erfasst. Beachten Sie, dass Sie das ONTAP-System nicht nur auf der Cloud Insights-Seite aktivieren, sondern auch so konfigurieren müssen, dass es bereitgestellt wird link:https://docs.netapp.com/us-en/ontap-cli-98/system-switch-ethernet-create.html["Switch-Informationen"]Und sorgen Sie für die richtige <<a-note-about-permissions,Berechtigungen>> Werden eingestellt, um die Switch-Daten an Cloud Insights senden zu können. Siehe „Ein Hinweis zu Berechtigungen“ weiter unten. 
|===
...
NOTE: Wenn die ONTAP-Zugriffssteuerung nicht korrekt eingestellt ist, können Cloud Insights-REST-Anrufe fehlschlagen, was zu Datenlücken für das Gerät führt.  Wenn Sie sie beispielsweise auf dem Cloud Insights Collector aktiviert haben, die Berechtigungen jedoch nicht auf dem ONTAP konfiguriert haben, schlägt die Akquisition fehl.  Wenn die Rolle zuvor auf der ONTAP definiert ist und Sie die Rest-API-Fähigkeiten hinzufügen, stellen Sie außerdem sicher, dass _http_ der Rolle hinzugefügt wird.
